...
import {
  ...
  PURGE_PRESETS,
  PurgePresetId,
} from "@components/internal/purge/purge.presets";
import {
  Card,
  CardContent,
  Chip,
...
export function PurgePresetCard({ onChange }: PurgePresetCardProps) {
  ...
  const matchNames = useMemo(() => {
    ...
    const names = new Set(
      PURGE_PRESETS.filter((preset) => selected.has(preset.id)).flatMap((preset) => preset.targets),
    );
        ...
        <Stack spacing={2} direction={{ xs: "column", xl: "row" }}>
          ...
          <Stack sx={{ width: { xs: "100%", xl: 280 }, flexShrink: 0 }}>
            <FormControlLabel
              control={
                <Switch
                  checked={estimateSizes}
                  onChange={(event) => setEstimateSizes(event.target.checked)}
                />
              }
