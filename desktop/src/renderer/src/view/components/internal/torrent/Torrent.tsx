...
import {
  ...
  sendTorrent,
  sendTorrentGroup,
} from "@modules/torrent/torrent.async.actions";
...
export function Torrent() {
  ...
  const { query, results, loading, sendStatuses, parseEpisodeInfos } = useAppSelector(
    (s) => s.torrent,
  );
  ...
  const columns = useMemo<GridColDef<NyaaTorrentItem>[]>(
    () => [
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <TorrentRowAction row={row} sendToQbittorrent={sendToQbittorrent} />
        ),
      },
    ],
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <TorrentGroupAction
            row={row}
            onClick={sendGroupToQbittorrent}
    ...
    <Stack className={"Torrent"}>
        ...
        <form onSubmit={search}>
          <Stack spacing={1}>
            ...
            >
              ...
              <Autocomplete
                renderInput={(params) => (
                  <TextField {...params} size={"small"} label={"Resolution"} />
                )}
                options={resolutions}
            ...
            >
              <FormControlLabel
                ...
                label="vostfr"
                slotProps={{ typography: { sx: { fontSize: "0.8125rem" } } }}
              />
              <FormControlLabel
      ...
      {torrents.length > 0 && (
        <Box className={"Torrent__status"}>
          <Typography
            variant="caption"
            sx={{ color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: "0.7rem" }}
          >
