...
import { FrameConfiguration } from "@shared/config/app.config";
				...
				<Stack direction="column" spacing={2}>
					<Stack direction="column" spacing={2}>
						<List subheader={<ListSubheader color={"primary"}>Frame</ListSubheader>}>
							<ListItem>
								<ListItemText
									primary="Show resource utilization"
									secondary={<Switch edge="end" checked={config.frame.show.resourceUtilization} onChange={(e) => toggleResources(e.target.checked)} />}
								/>
							</ListItem>
						</List>
								...
								<ListItem key={key}>
									<ListItemText
										...
										secondary={
											<Switch
												edge="end"
												checked={config.frame.resize[key as keyof FrameConfiguration["resize"]]}
												onChange={(e) => toggleResize(key as keyof FrameConfiguration["resize"], e.target.checked)}
											/>
										}
									/>
								</ListItem>
