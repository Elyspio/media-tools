...
import {
  ...
  setFileProcesses,
  setProcessProgress,
} from "@modules/encoder/encoder.reducer";
 ...
 */
export const ignoreAlreadyConvertedFiles = createAsyncThunk(
  "skip-already-converted",
  ({ files, format }: IgnoreAlreadyConvertedFilesParams, { dispatch }) => {
    let alreadyConverted: Media[] = [];

    if (format.includes("hevc")) {
      alreadyConverted = files.filter((f) =>
        f.property.streams.find((s) => s.codec_name === "hevc"),
      );
    }

    if (format.includes("h264")) {
      alreadyConverted = files.filter((f) =>
        f.property.streams.find((s) => s.codec_name === "h264"),
      );
    }

    if (format.includes("av1")) {
      alreadyConverted = files.filter((f) =>
        f.property.streams.find((s) => s.codec_name === "av1"),
      );
    }
    ...
    await Promise.allSettled(
      conversionBatches.map(async (batch) => {
        for (const file of batch.filter((f) => !alreadyConverted.includes(f))) {
          ...
          const nbFrames =
            sourceFps > 0 ? Math.floor(sourceNbFrames * (fps / sourceFps)) : sourceNbFrames;
...
export const stopConvertMedia = createAsyncThunk(
  "stop-convert",
  async (_, { dispatch, getState, extra }) => {
    ...
    for (const pid of pids) {
      ...
      const currentPath = Object.entries(state.encoder.processes.pids).find(
        ([, p]) => p === pid,
      )?.[0];
...
export const setupFfmpeg = createAsyncThunk(
  "ffmpeg/setup",
  async (): Promise<{ isAvailable: boolean; encoders?: Encoder[] }> => {
