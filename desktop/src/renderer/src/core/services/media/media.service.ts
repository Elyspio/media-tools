import { File, Media, MediaData } from "@components/internal/encoder/type";
import { setFFmpegInstalled } from "@modules/media/media.action";
...
export class MediaService {
	...
	public async getInfo(file: File): Promise<MediaData> {
		try {
			const { stdout } = await this.processService.exec(`ffprobe.exe  -v quiet -print_format json -show_format -show_streams "${file.path}"`);
	...
	public async convert(
		...
		const outputPath = options?.outputPath ?? (await window.preload.ipc.send.file.getRandomTempFilePath(`output.mkv`));
		const pid = await this.processService.spawn("ffmpeg.exe", ["-y", "-i", input.file.path, "-map", "0", "-c:v", format, "-progress", "-", "-nostats", outputPath]);
	...
	public getFFMpegProgress(chunk: string): FFMPpegProgress {
		const data = chunk
			.toString()
			.split("\n")
			.map((line) => line.split("="));
