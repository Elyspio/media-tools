...
export class IpcModule extends LogModule {
  public constructor(
    @inject(new LazyServiceIdentifier(() => WindowModule))
  ...
  public sendIpcToWebContent<Channel extends keyof IpcSentEvents>(
    content: WebContents,
    channel: Channel,
    ...args: SkipFirst<Parameters<IpcSentEvents[Channel]>>
  ) {
    this.logger.debug("Sending IPC to web content", content.id, channel, ...args);
    content.send(channel, ...args);
  }

  public sendIpcToMainContent<Channel extends keyof IpcSentEvents>(
    channel: Channel,
    ...args: SkipFirst<Parameters<IpcSentEvents[Channel]>>
  ) {
