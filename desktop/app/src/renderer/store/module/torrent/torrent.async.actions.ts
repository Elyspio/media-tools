...
export const startQBittorrent = createAsyncThunk("start-qbittorrent", async (_, { extra, dispatch }) => {
	{
		const services = getServices({
...
export const updateTorrent = createAsyncThunk("update", async ({ mode, hash }: UpdateTorrentParams, {
	extra,
	dispatch,
}) => {
	const torrentService = getService(TorrentService, extra);
