...
import { RenamerState } from "../../../renderer/store/module/renamer/renamer.types";
...
export class RenamerService {
	 ...
	 * @param newName - The new name to be used in the filename
	 * @param max - The maximum number to use for the file renaming
	 * @param files - The collection of files to rename
	 * @return A Promise that resolves once all files have been renamed
	 */
	public async rename(newName: string, max: number, files: RenamerState["files"]): Promise<any> {
		if (!(newName !== undefined && newName?.length > 0)) throw new Error("New name is empty");

		return Promise.all(
			files.map((file) => {
				return async () => {
					const num = this.stringService.padWithZeros(file.num, max!.toString().length);
					await fs.rename(file.name, this.getNewEpisodeName(newName, file, num));
				};
			})
		);
	}
