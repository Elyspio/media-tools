...
class Application extends React.Component<ReduxTypes> {
	...
	checkHeight = async () => {
		...
		if (this.props.current.autoResize.width || this.props.current.autoResize.height) {
			...
			// @ts-ignore
			const dim: (keyof Configuration["frame"]["resize"])[] = [...Object.keys(config.frame.resize)].filter(k => config.frame.resize[k] && this.props.current.autoResize[k]);
			console.log(BrowserWindow.fromId(1).getSize());
