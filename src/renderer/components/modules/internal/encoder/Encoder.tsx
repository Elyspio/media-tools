...
function Encoder() {
	...
	const services = {
		files: useInjection<FilesService>(DependencyInjectionKeys.files), media: useInjection<MediaService>(DependencyInjectionKeys.media.convert)
	};
	...
	const setProcesses = React.useCallback((media?: Media[]) => {
	...
	const encodeFile = React.useCallback((media: Media): Promise<string> => {
	...
	const {optionsUi, actionsUi} = React.useMemo(() => {
		...
		const actionsUi = <Grid container className="actions" spacing={4}>
			<Grid item>
		...
		return {
			actionsUi: medias.length ? actionsUi : null, optionsUi: medias.length ? optionsUi : null
		};
